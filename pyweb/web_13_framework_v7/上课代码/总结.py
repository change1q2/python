...
3， 自定义的错误信息，可以直接通过逗号放到后面， assert 1 == 2, "错误信息"
...
@pytest.mark.paramatrize("参数名称", test_data)
def test_login(参数名称):
    pass
...
allure 测试报告：
1， python, java , 都是可以用的


allure 报高安装步骤：
1， 下载 安装包，解压， 添加 allure 环境变量；
2， 安装 allure-pytest 库
3， 使用 pytest --alluredir= 生成测试报告
4， 调用 allure serve 路径  查看报告

jenkins 配置， 替代 第4 步
- 安装 jekins 的 allure 插件
- 2， 配置 jenkins 全局工具路径， allure 包安装的路径
- 3， 项目当中 构建后操作 添加 allure  测试报告路径。
